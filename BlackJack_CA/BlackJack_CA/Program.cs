...
namespace BlackJack_CA
{
    internal class Program
    {
        ...
        static int[] cardsDrawn()
        {
            ...
            int[] handsvalue=new int[2];//Player is the first value and the Dealer is the second value 


            //Player goes first
            while (p_num_ofcards <= 1) //Gets the first two cards for the player
            {
                Thread.Sleep(1000);//Allows the game to flow naturally
                ...
                ConfirmPlayerAce(card);//Checks for an ace
            ...
            //Dealer goes next
            while (d_num_ofcards < 1) //Gets the first card for the dealer
            {
                Thread.Sleep(1000);//Allows the game to flow naturally
...
;
        ...
        static int DealerPlays(int dealersHandValue)
        {
            while (dealersHandValue < 17)
            {
                ...
                d_num_ofcards++;//Amount of cards the dealer has increases

                ConfirmDealerAce(card);//Checks for an ace
